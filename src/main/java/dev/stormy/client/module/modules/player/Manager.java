...
public class Manager extends Module {
	...
	public Manager() {
		...
		this.registerSetting(delay = new DoubleSliderSetting("Delay", 50, 100, 50, 500, 50));
		this.registerSetting(swordSlot = new SliderSetting("Sword Slot", 2, 1, 9, 1));
		this.registerSetting(blockSlot = new SliderSetting("Blocks Slot", 8, 1, 9, 1));
	...
	public void onUpdate(UpdateEvent event) {
		if (!event.isPre() || (mc.thePlayer.ticksExisted <= 40)) {
			return;
		}
		...
		if (this.stopwatch.hasReached(this.nextClick) || this.chestTicks < 10) {
			this.closeInventory();
		}
			...
			if (item instanceof ItemPotion) {
				if (potion == -1) {
					...
					if (itemPotion.getEffects(stack) != null && currentItemPotion.getEffects(currentStack) != null) {
						if ((PotionUtils.potionRanking(itemPotion.getEffects(stack).get(0).getPotionID()) > PotionUtils.potionRanking(currentItemPotion.getEffects(currentStack).get(0).getPotionID()) || foundCurrent) && !found) {
							potion = i;
						}
					}
				}
